...
export default class CustomSearchLookup extends LightningElement {
    ...
    @wire(fetchLookupData, { searchKey: '$searchKey' , sObjectApiName : '$sObjectApiName', accountRecordType: '$accountRecordType' })
    searchResult(value) {
...
// send selected lookup record to parent component using custom event
    ...
    @api
    handleSetError() {
        let itemDD = this.template.querySelector(".linput");

        if (itemDD.required) {
            itemDD.setCustomValidity('Please, fill this field');
            itemDD.reportValidity();
        }
    }

    @api
    handleSetErrorExistingLookup() {
        let itemDD = this.template.querySelector(".linput");
    ...
    @api
    handleSetErrorExistingLookupContact() {
        let itemDD = this.template.querySelector(".linput");
    ...
    @api
    removeSetErrorExistingLookup() {
        let itemDD = this.template.querySelector(".linput");

        itemDD.setCustomValidity('');
        itemDD.reportValidity();
    }

    @api
    removeSetErrorExistingLookupContact() {
        let itemDD = this.template.querySelector(".linput");
